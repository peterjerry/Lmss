        ...
        ngx_chain_t *in)
{
    ...
    if (!ngx_rtmp_remote_conf()) {
        ...
        if (ngx_rtmp_cmd_get_core_srv_conf(s, NGX_RTMP_CMD_RTMP_PUBLISH, &s->host_in, &s->app, &cscf, &cacf) != NGX_OK) {
        ...
        ngx_chain_t *in)
{
    ...
    if (!ngx_rtmp_remote_conf()) {
        ...
        if (s->relay_type == NGX_NONE_RELAY) {
            ...
            if (ngx_rtmp_cmd_get_core_srv_conf(s, NGX_RTMP_CMD_RTMP_PLAY, &s->host_in, &s->app, &cscf, &cacf) != NGX_OK) {
...
{
    ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
		"start_hls_slice: vhost='%V' app='%V' name='%V'", &s->host_in, &s->app, &s->name);
