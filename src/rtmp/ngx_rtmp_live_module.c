...
ngx_rtmp_live_checking_callback(ngx_event_t *e)
{
	...
	*ngx_cpymem(v.name, s->name, ngx_strlen(s->name)) = 0;
...
{
    ngx_rtmp_live_stream_t **  stream = ngx_rtmp_live_get_stream(s, name, 0);
    if (stream != NULL && (*stream)->publishing){
        return NGX_OK;
    }
    return NGX_ERROR;
}
...
ngx_rtmp_live_stream_begin(ngx_rtmp_session_t *s, ngx_rtmp_stream_begin_t *v)
{
    ...
    if (ctx == NULL || ctx->stream == NULL || !ctx->publishing || ctx->hls) {
        goto next;
    }
...
ngx_rtmp_live_get_stream_by_session(ngx_rtmp_session_t *s)
{
    ...
    stream = ngx_rtmp_live_get_stream(s, s->name, 0);
...
ngx_rtmp_live_pause(ngx_rtmp_session_t *s, ngx_rtmp_pause_t *v)
{
    ...
    if (ctx == NULL || ctx->stream == NULL || ctx->hls) {
        goto next;
    }
...
*/
    ...
    for (pctx = ctx->stream->ctx; pctx; pctx = pctx->next) {
        if (pctx == ctx || pctx->paused || pctx->hls) {
            continue;
        }
...
ngx_rtmp_live_play(ngx_rtmp_session_t *s, ngx_rtmp_play_t *v)
{
    ...
    if (!ctx->silent && !lacf->play_restart && !ctx->hls) {
