        ...
        ngx_rtmp_vost_stat_t *vhost_stat, ngx_rtmp_stat_para_t *para)
{
    ...
    ngx_rtmp_live_ctx_t            *ctx, *ctx1;
    ngx_rtmp_session_t             *s, *s1;
    ngx_int_t                       n;
    ...
    for (n = 0; n < nbuckets; ++n) {
        ...
        for (; stream; stream = stream->next) {
            ...
            if (codec) {
	            ...
	            vhost_stat->vhost_bd_in.bandwidth += stream->bw_in.bandwidth;
	            vhost_stat->vhost_bd_in.bytes += stream->bw_in.bytes;
	            vhost_stat->vhost_bd_out.bandwidth += stream->bw_out.bandwidth;
	            vhost_stat->vhost_bd_out.bytes += stream->bw_out.bytes;
	            vhost_stat->vhost_bd_real.bandwidth += stream->bw_real.bandwidth;
	            vhost_stat->vhost_bd_real.bytes += stream->bw_real.bytes;
